    ...
    if ($_GET['q'] && $_GET['p'] && $_GET['varausid'])
    {
        ...
        else // lisäpalvelut
        {
            foreach ($palvelut as $palvelu)
            {
                ...
                echo "<li>" . $palvelu->getNimi() . "<input type='text' maxlength=2 class='textinput' style='width:4em' name='lisapalvelu" . $palvelu->getPalveluId() . "' value='" . $tk->HaeVaraukseenKuuluvanPalvelunLukumaara($varausid, $palvelu->getPalveluId()) ."' /> kpl</li>\n";
